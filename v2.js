	...
	if (productIdentities) {
		...
		if (components.size()>0) {	
				...
				// calculate summary of procent 
											...
											for (var t=0; t< maxLinesInRow(itemId, components.get(0)); t++ ){
												
												countProc=countProc+Number(getElement(itemId,((components.get(0) - ATTRIBUTE_IDENTIFIER_MAT_COMP_START) * ATTRIBUTE_IDENTIFIER_MAT_COMP_COUNT + ATTRIBUTE_IDENTIFIER_MAT_FRA_START + t)));
											};
											
											//------------------------------------------
													
				content.
					append(getTableHeader(countProc));
...
function getTableHeader(percent) {
	...
	if (percent == 100){
	header.append("<td width='150'><font face='Verdana'><b>Anteil</b></font></td>");
	} else {
		header.
		append("<td width='150'><font face='Verdana'><b>Anteil </b></font> <font face='Verdana' color='red'><b> (").
		append(percent).
		append("%)</b></font></td>");
	};
	header.
