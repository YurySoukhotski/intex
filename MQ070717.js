 ...
 */
/**
 * This script will be executed on pim2 product creation. It will set the user
 * and the current date to pim2 attributes
 * 
 */
...
function processMessage(objectMessage, entryFacade) {
	...
	if (!loggerExists) {
		...
		var layout = new Log4j.PatternLayout(
				"%d [%t] %-5p %C.%M() %c{1} - %m%n");
	...
	var processProduct = function(productIdentifier, userName) {
		...
		logger.info("Set user=" + userName + " and date="
				+ dateAsTimestampInMilli);
		...
		var neuQuery = "WITH rec(ID, PARENT_ID) AS (SELECT ID, parent_id FROM PIM2_PRODUCT WHERE id ="
				+ itemId + " UNION ALL SELECT rec1.ID, rec1.parent_id FROM PIM2_PRODUCT rec1, rec WHERE rec1.id =rec.PARENT_ID) SELECT ID FROM rec where PARENT_ID is null";
		...
		var findRootQuery = "WITH rec(id, identifier, parent_id) AS (SELECT id, identifier, parent_id FROM pim2_node WHERE id IN (SELECT node_id FROM pim2_product_2_node WHERE product_id ="
				+ list.get(0)+ ") UNION ALL SELECT rec1.id, rec1.identifier, rec1.parent_id FROM pim2_node rec1, rec WHERE rec1.id=rec.parent_id) SELECT identifier FROM rec where parent_id is null";
		...
		groupedProductsForUpdate.put(
				new Packages.com.meylemueller.pim2.identity.ItemIdentity(
						productIdentifier), updatedValues);
	...
	// because it take some time until the item is available via API, it has to
	// wait
